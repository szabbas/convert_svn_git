...
outer:		for(EFmFmEmployeeTravelRequestPO cabRequests:travelDetails){
		...
		public Response algoCaballocation(EFmFmEmployeeTravelRequestPO travelRequestPO) throws ParseException {			
			...
			Date shift = (Date) shiftFormate.parse(shiftTime);
			Date today = new Date();			
			System.out.println("inova"+travelRequestPO.getInnovaCount());
			System.out.println("tempo"+travelRequestPO.getTempoCount());
			System.out.println("excution"+travelRequestPO.getTypeExecution());
			System.out.println("Date:"+travelRequestPO.getExecutionDate());
			System.out.println("employeeCount:"+travelRequestPO.getEmployeeCount());
			...
			List<EFmFmEmployeeTravelRequestPO> travelDetails = iCabRequestBO
					.listOfTravelRequestByShiftTripType(travelRequestPO.getBranchId(), dateShiftTime,
							travelRequestPO.getTripType(),convertedCurrentDate);
			...
			if (vehicleDetails.size() > 0) {
				for (EFmFmVehicleMasterPO vehicleModelList : vehicleDetails) {
						...
						vehicleModelList.getVehicleModel(),
						vehicleModelList.getAvailableSeat());
					vehicleTypes.add(V);					
					if (V.capacity < 6) {
			...
			//settings for algorithm, make changes here for now to use different constraints
			...
			problem.settings.max_travel_time = 7200;
