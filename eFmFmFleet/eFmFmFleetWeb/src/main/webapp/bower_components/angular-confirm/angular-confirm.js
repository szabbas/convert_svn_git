...
angular.module('angular-confirm', ['ui.bootstrap'])
  ...
  .value('$confirmModalDefaults', {
    template: '<div class="modal-header confirmDialogCustomHeader"><h3 class="modal-title">{{data.title}}</h3></div>' +
    '<div class="modal-body confirmDialogCustomBody">{{data.text}}</div>' +
    '<div class="modal-footer confirmDialogFooter">' +
    '<button class="btn btn-success buttonRadius0" ng-click="ok()">{{data.ok}}</button>' +
    '<button class="btn btn-danger buttonRadius0" ng-click="cancel()">{{data.cancel}}</button>' +
    '</div>',
