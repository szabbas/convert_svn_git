...
//					if(!(tripDates.contains(tripDate))){
					...
					log.info("vehicleDetails:"+vehicleDetails.size());
			...
			for (Date tripDetails : selectedDates) {
				...
				for (EFmFmAssignRoutePO delayTrips : trips) {
					...
					if (assignRoutePO.getTripType().equalsIgnoreCase("DROP")) {
						...
						else if((shiftDateAndTime.getTime()+onTime)<delayTrips.getTripStartTime().getTime()){
						...
						else if((shiftDateAndTime.getTime()+onTime)<delayTrips.getTripCompleteTime().getTime()){
							delayBeyondTimeCount++;
						}
						else{
							delayTripCount++;
