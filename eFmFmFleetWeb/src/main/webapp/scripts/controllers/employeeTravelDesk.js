...
var assignCabCtrl = function($scope, $modalInstance, $state, $http, $timeout){  
       ...
       $scope.openAssignDateCal = function($event){
            ...
            $timeout( function(){
               $scope.datePicker = {'openedassignDate' : true};  
            })
        };
       ...
       $http.post('services/zones/createbucket/',data).
            success(function(data, status, headers, config) {
                ...
                $scope.setTripType($scope.assignCab.tripType);
    ...
    $scope.applyAlgorithm = function(tripType, shiftType){ 
        //alert($scope.assignCab.assignDate);
      ...
      if(!$scope.assignCab.needRoutewise){
        ...
        if(angular.isObject($scope.assignCab.assignDate)){
           ...
           $http.post('services/xlEmployeeExport/algoPopUpInputs/', dataObj).
              success(function(data, status, headers, config) {
                 //alert(JSON.stringify(data));
                  $scope.assignCab.assignDate = new Date();
                  ...
                  //alert(JSON.stringify($scope.assignCabsTempo));
    ...
    $scope.assign = function(cab){
        ...
        if(cab.shiftTime == 'preDefineShiftTime'){
 ...
 //        alert(tripTimeSelected);
        ...
        if(cab.applyAlgorithm){        	
           /* alert("TRIP TYPE: "+ cab.tripType.value + " - " + 
                  "SHIFT TIME: " + cab.shiftTime.shiftTime+" - " + 
                  "APPLY ALGORITHM: " +cab.applyAlgorithm+ " - " + 
                  "ALGO DATE: " + convertDateUTC(cab.assignDate) + " - " + 
                  "VEHICLE TEMPO: " + cab.vehicleTempo.valueList + " - " + 
                  "VEHICLE INOVA: " +  cab.vehicleInova.valueList + " - " + 
                  "NUMBER OF EMPLOYEE: " + cab.noOfEmp + " - " + 
                  "ASSIGN CAB: " + cab.needRoutewise+" - " + 
                  "ZONE ID: " + selectedZone);*/
            
            if((cab.vehicleInova.valueList ==0 && cab.vehicleTempo.valueList ==0) || cab.noOfEmp ==0){
            	
                $scope.showalertMessageModal("Could not apply algorithm process. Please change your selections.");
                
            }else{            	
            var dataObj = {
                    branchId:branchId,
                    time:cab.shiftTime.shiftTime, // replaced the *time:cab.shiftTime.shiftTime,
                    zoneId:selectedZone,
                    tripType:cab.tripType.value,                   
                    executionDate:convertDateUTC(cab.assignDate),
                    employeeCount:cab.noOfEmp,
                    innovaCount:parseInt(cab.vehicleInova.valueList),
                    tempoCount:parseInt(cab.vehicleTempo.valueList),
                    typeExecution:cab.applyAlgorithm
                  };     
            
            $http.post('services/request/algoCaballocation/',dataObj).
                success(function(data, status, headers, config) {
                 // alert(JSON.stringify(data));
                  if(data==""){                          
                        $scope.showalertMessageModal("Algoritham Cab allocation is in progress....", ''); 
